...
modbus::modbus() : Baudrate(19200), LastTxLiveData(0), LastTxIdData(0), LastTxInverter(0), pin_RX(16), pin_TX(17), pin_RTS(18) {
  #ifdef JSON_MQTT
  this->mqttJson.reserve(JSON_MSG_BUFFER);  // pre-allocate memory for the JSON to get rid of dynamic allocation
  #endif
...
void modbus::init(bool firstrun) {
  ...
  // factor for transmission-timings depending on the baudrate:
  this->BpsFactor = (9600 / this->Baudrate) + 1;    // we have a byte-transmission-duration of 1ms when baudrate is 10000bps at 8N1 
  if (this->BpsFactor < 1) {this->BpsFactor = 1;}   // ->waiting for next char to be fully received: 1ms@19200bps, 2ms@9600bps, 3ms@4800bps, 5ms@2400bps, 9ms@1200bps...
  if (Config->GetDebugLevel() >=3) {dbg.printf("Calculated BpsFactor for serial timings:%d\n", this->BpsFactor);}
...
void modbus::QueryQueueToInverter() {
  ...
  if (rwtype != NUL) {
    ...
    RS485Serial->flush();
    digitalWrite(this->pin_RTS, RS485Receive);      // after transmission set to receive

    unsigned long start   = millis();
    unsigned long timeout = start + RS485_READ_TIMEOUT; // wait for receiving the first char (with timeout)
    while (millis()<=timeout && (!RS485Serial->available())) {yield();}
    if (Config->GetDebugLevel() >=3) {dbg.printf("Time waiting for receiving chars %d ms\n", millis()-start);}

    start   = millis();
    timeout = start + (50 * this->BpsFactor);       // wait for buffer to fill (if chars were received) - getting roundabout 50 chars
    while (millis()<=timeout && (RS485Serial->available())) {yield();}
    if (Config->GetDebugLevel() >=3) {dbg.printf("Time waiting to fill char-buffer %d ms\n", millis()-start);}
...
bool modbus::ReceiveSetData(std::vector<byte>* SendHexFrame) {
  ...
  //digitalWrite(this->pin_RTS, RS485Receive);     // init Receive >>> löschen, umschalten direkt vor available() ist zu spät (und nicht nötig)
  if (RS485Serial->available()) {
    ...
    while(RS485Serial->available()) {
      ...
      delay(1 * this->BpsFactor);                 // keep this! Loosing bytes possible if too fast. Waiting for transmission of 1 char.
    }    
...
void modbus::ReceiveReadData() {
  ...
  if (RS485Serial->available()) {
    ...
    while(RS485Serial->available()) {
      ...
      if (Config->GetDebugLevel() >=4) {dbg.print(PrintHex(d)); dbg.print(" ");}
      delay(1 * this->BpsFactor);                 // keep this! Loosing bytes possible if too fast
    }
    if (Config->GetDebugLevel() >=4) {dbg.println();}
...
void modbus::ParseData() {
  ...
  if (this->DataFrame->size() >= 1) {
    ...
    // setup RequestType
    if      (tempbyte == DATAISID  ) {RequestType = "id";}
    else if (tempbyte == DATAISLIVE) {RequestType = "livedata";}
  }
  ...
  if (this->DataFrame->size() <= 1) {
    ...
    //  ***********************************************
      ...
      if(tempbyte == DATAISID){
        // one id-telegram of 65 byte
        byte ReadBufID[] = {0x01,0x03,0x3C,0x00,0x01,0xA9,0x48,0x00,0x00,0x00,0x64,0x00,0x64,0x27,0x10,0x00,0x00,0x27,0x10,0x0E,0x10,0x59,0x41,0x31,0x2E,0x30,0x00,0x5A,0x44,0x41,0x41,0x00,0x09,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x3C,0x00,0x3C,0x00,0x64,0x00,0x64,0x00,0x00,0x53,0x4E,0x31,0x32,0x30,0x39,0x31,0x39,0x36,0x34,0x11,0x03,0xF8,0xAD,0x7A,0xA7};

        for (uint8_t i = 0; i<sizeof(ReadBufID); i++) {
          this->DataFrame->push_back(ReadBufID[i]);
          // if (Config->GetDebugLevel() >=4) {Serial.print(PrintHex(ReadBufID[i])); Serial.print(" ");}
        }
      }else if(tempbyte == DATAISLIVE){
        //Growatt LiveData
        // two livedata-telegrams, 243 and 255 byte long
        byte ReadBufLive1[] = {};
        byte ReadBufLive2[] = {};
        for (uint8_t i = 0; i<sizeof(ReadBufLive1); i++) {this->DataFrame->push_back(ReadBufLive1[i]);}
        for (uint8_t i = 0; i<sizeof(ReadBufLive2); i++) {this->DataFrame->push_back(ReadBufLive2[i]);}
      }
  ...
  if (this->DataFrame->size() >= 1) {
    ...
    if (Config->GetDebugLevel() >=3) dbg.printf("identified datatype: %s\n", RequestType.c_str());
    ...
    do {
      ...
      if (datatype == "float") {
        ...
        if (!posArray.isNull()) { 
          ...
          uint8_t i=0;
          for(int v : posArray) {
            buf[i] = (char)DataFrame->at(v);
            i++;
          }
          buf[i] = (char)0x00;                // end c-string with 0x00
          d.value = String(buf);
        } 
      } else {
        //****************** sonst, leer *******************//
      ...
      if (this->mqtt && IsActiveItem && d.Name != "") { 
          ...
          sprintf(buffer, "\"%s\":\"%s\"", d.Name.c_str(), d.value.c_str());
          this->mqttJson += buffer;
        #else
    ...
    #ifdef JSON_MQTT
      // append relay-states to JSON-answer for livedata
      if (RequestType == "livedata" && this->enableRelays) {
        if (noData) {noData = false;}
        else        {this->mqttJson += ",";}

        this->state_Relay1 = digitalRead(this->pin_Relay1);    
        sprintf(buffer, "\"relay1\":\"%d\"", this->state_Relay1);
        this->mqttJson += buffer;

        this->state_Relay2 = digitalRead(this->pin_Relay2);    
        sprintf(buffer, ",\"relay2\":\"%d\"", this->state_Relay2);
        this->mqttJson += buffer;
      }

      this->mqttJson += "}";
      if (this->mqtt && !noData) {
        this->mqtt->Publish_String((char*)RequestType.c_str(), this->mqttJson, false);
      }
    #endif
  }
...
String modbus::MapItem(JsonArray map, String value) {
  ...
  for (JsonArray mapItem : map) {
    ...
    if (value == v1) {
      ret = v2;
      if (Config->GetDebugLevel() >=4) dbg.printf("Mapped value: %s -> %s\n", v1.c_str(), v2.c_str());
    }
  } 
  return ret;
}
